...
<details>
<summary>Что такое Change Detection, Change Detection Engine?</summary>
<div>
...
<div>
<br>Конструктор сам по себе является фичей самого класса, а не Angular. Основная разница в том, что конструктор будет вызван всего один раз, на этапе инициализации компонентов в модуле, ngOnInit, в свою очередь, будет вызываться каждый раз, когда пользователь, условно говоря, получает доступ к компоненту (например переходит по определенному URL).
</div><br>
</details>
