...
#### Какие есть главная и поперечная оси во Flexbox?
...
#### Когда лучше использовать Flexbox, а когда Grid?
...
#### Чем `gap` лучше margin для расстояний между элементами?
...
#### Почему CSS может блокировать первый render?
...
#### Почему синхронный JavaScript может блокировать parsing HTML?
...
#### Какие задачи конкурируют за main thread?
...
#### Что такое debounce и throttle с точки зрения performance?
...
#### Что такое virtualization списков?
...
#### Что такое `srcset` и responsive images?
...
#### Что такое preload, prefetch и preconnect?
...
#### Чем Lighthouse отличается от реальных пользовательских метрик?
...
#### Чем отличаются 2xx, 3xx, 4xx и 5xx?
...
#### Какие способы управления состоянием есть в Angular?
...
#### Чем Signals отличаются от Redux-подхода?
...
#### Чем NgRx отличается от обычного сервиса?
...
#### Что является артефактом frontend build?
...
#### Чем dev server отличается от production hosting?
...
#### Где обычно хранят production frontend assets?
...
#### Почему assets часто кешируют долго?
...
#### Почему `index.html` обычно кешируют осторожнее?
...
#### Как hash в имени файла помогает кешированию?
...
#### Как environment variables попадают во frontend?
...
#### Почему runtime config иногда лучше build-time config?
...
#### Какие метрики важно смотреть после деплоя frontend?
...
<details>
<summary>Почему `bootstrapApplication` на сервере требует `BootstrapContext`?</summary>
...
<details>
<summary>Чем `main.ts` отличается от `main.server.ts`?</summary>
