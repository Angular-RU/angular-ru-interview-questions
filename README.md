...
<summary>Что такое интерполяция в Angular?</summary>
  ...
  ```
  Принцип работы: во время процесса компиляции компилятор использует **$interpolate** для анализа, содержит ли текстовая нода   или значение аттрибута разметку интерполяции. В случае, если выражение присутствует, компилятор добавляет   **interpolateDirective** и регистритует слушатель (*watches*) на скомпилированную функцию интерполяции, которая обновит соотвествующую текстовую ноду или значение аттрибута как часть обычноного **digest** цикла.
Следует обратить внимание, что **interpolateDirective** имеет приоритет 100 и устанавливает слушатель в **preLink** функции. <br>
</details>
