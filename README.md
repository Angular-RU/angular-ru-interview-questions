...
Внедрение зависимости - процесс предоставления внешней зависимости программному компоненту. DI (Dependency Injection) привязан к Angular фреймворку и используется везде, где необходимо внедрить новые компоненты с сервисами или другими необходимыми элементами. Компоненты “поглощают” сервисы, это значит что Вы можете “внедрять” (inject) сервис в компонент, предоставляя компоненту доступ к соответственному классу сервиса.
Для определения класса как сервиса в Angular используется декоратор `@Injectable` для внедрения метаданных которая позволяет внедрить ее в компонент как зависимость.

Когда Angular создает новой екземпляр класса компонента, он определяет, какие сервисы или другие зависимости необходимы компоненту, проверяя типы в параметрах конструктора.

Когда Angular находит компонент который имеет зависимость на сервис, преджде всего фреймворк проверяет, сущевтсвуют ли уже екземпляры сервиса. Если запрошенный екзепляр сервиса еще не создан, инжектор (injector) создает новый, используя зарегисттированный провайдер и затем добавляет его в инжектор перед возвратом сервиса в Angular.
