...
```html
<section [@panel]="isOpen() ? 'expanded' : 'collapsed'"></section>
```

Для современного Angular стоит отдельно проговорить, что legacy trigger API может встретиться при поддержке старого
кода, а новые enter/leave и state-driven animation cases часто проще решить CSS transition, Web Animations API или
анимациями UI-kit.
...
```ts
transition('* => expanded', animate('200ms'));
transition('collapsed => *', animate('150ms'));
transition('* => *', animate('120ms'));
```

Такой transition удобен для fallback-логики, но в сложных компонентах может скрывать реальные состояния. Лучше явно
называть важные UI states и оставлять wildcard только для универсального поведения.
...
На интервью важно объяснить не только синтаксис, но и жизненный цикл: Angular применяет trigger к элементу, сравнивает
предыдущее и новое состояние, затем запускает подходящий transition.
...
```ts
const requests$ = this.query.valueChanges.pipe(map((query) => this.api.search(query)));
```

С таким stream почти всегда нужен flattening operator: `switchMap`, `concatMap`, `mergeMap` или `exhaustMap`. Выбор
зависит от бизнес-смысла: отменять старое, выполнять параллельно, сохранять порядок или игнорировать новые события.
...
В Angular чаще всего встречается `share()` или `shareReplay()`, но их нельзя применять механически:

- `share()` не кеширует прошлое значение;
- `shareReplay()` может удерживать значение в памяти;
- без `refCount` долгоживущий source может не остановиться, когда подписчиков больше нет.

Хороший ответ объясняет, что именно шарится: результат HTTP-запроса, WebSocket connection, expensive computation или
template-facing stream.
...
```ts
of([1, 2, 3]); // один emission: массив
from([1, 2, 3]); // три emission: 1, 2, 3
from(fetch('/api/users')); // emission результата Promise
```

На практике `of(fallback)` часто используют в `catchError`, а `from(promise)` нужен, когда Promise надо включить в RxJS
pipeline.
...
```ts
zip(userIds$, permissions$).subscribe(([userId, permissions]) => {
  // значения синхронизированы по позиции, а не по времени последнего изменения
});
```

Для UI чаще подходят `combineLatest`, `withLatestFrom` или `forkJoin`. `zip` уместен, когда streams представляют парные
последовательности и важно сопоставлять элементы именно по номеру emission.
...
У приложения может быть один основной outlet и named outlets для вспомогательных областей интерфейса. Важно не путать
`RouterOutlet` с lazy loading: outlet отвечает за место отображения, а lazy route - за момент загрузки route code.
...
`RouterState` описывает все дерево активных routes. Он нужен, когда важно видеть контекст навигации целиком: guards,
analytics, breadcrumbs, debug tooling или инфраструктурный routing code.

В компоненте обычно достаточно `ActivatedRoute`. Если приходится часто обходить весь `RouterState`, стоит проверить, не
смешана ли навигационная инфраструктура с feature UI.
...
- много derived values и связанных async commands;
- несколько distant features, которые пишут в одно состояние;
- требования к devtools, audit trail, undo/redo или replay;
- необходимость запретить внешнюю мутацию и формализовать update contract.

В modern Angular можно рассмотреть `@ngrx/signals` `signalStore()` как промежуточный вариант: структура store, computed
state и methods без полного Redux-style boilerplate. Classic NgRx выбирают, когда нужна вся дисциплина actions, reducers
и effects.
...
```txt
UserPageComponent
  читает route id, вызывает UserFacade
  передает userVm в UserCardComponent

UserCardComponent
  показывает userVm
  сообщает наружу о click/edit/delete
```

Такое разделение уменьшает связанность UI с data access и упрощает тесты. Это не догма: маленький компонент не нужно
насильно делить, если разделение не добавляет ясности.
...
Хороший пример: большой кабинет с правами пользователя, entity cache, websocket updates, drafts, optimistic save и
несколькими feature teams. В таком случае actions/selectors/effects дают общий язык и воспроизводимую отладку.

Плохой пример: простая форма или один экран с HTTP-запросом. Там `signal()` в компоненте, feature service или
`httpResource()` обычно проще и дешевле.
...
- route/container component связывает URL, permissions и feature lifecycle;
- facade или feature store владеет writable signals и commands;
- API service знает backend DTO и transport details;
- mapper переводит DTO в view/domain model;
- presentational components получают readonly data и events.

Важно не отдавать наружу writable signal без необходимости. Публичный API feature должен выражать намерения:
`selectUser(id)`, `saveDraft()`, `resetFilters()`, а не позволять любому consumer менять внутренние поля напрямую.
...
2. Перевести локальное синхронное состояние на `signal()` и `computed()`.
3. На границе Observable использовать `toSignal()` или оставить `async` pipe, если stream уже хорошо выражает async
   flow.
4. Новые inputs можно делать через `input()` там, где это не ломает consumers.
5. Удалять RxJS только там, где stream был заменой обычному synchronous state.

Главный критерий: код стал проще читать и тестировать. Если есть cancellation, debounce, WebSocket или сложная
конкуренция, RxJS может остаться правильным инструментом.
...
- писать side effects внутри `computed()`;
- мутировать объект или массив внутри signal in place;
- отдавать writable signal всем consumers;
- заменять RxJS signals там, где нужна отмена, backpressure или сложная async composition;
- делать один глобальный signal-store для несвязанных features;
- использовать `effect()` как скрытый orchestration layer вместо явных commands.

Хороший signal-код имеет чистые `computed()`, узкие commands, readonly public state и понятные границы ownership.
...
```ts filename="app.config.server.ts"
...
Лучшее решение почти всегда проще:
...
RxJS стоит оставить, если click участвует во времени: debounce, throttle, async request, cancellation, retry, WebSocket
или coordination с другими streams. Если это обычное synchronous UI state, signal или простое поле читаются лучше.
...
- search: `debounceTime`, `distinctUntilChanged`, `switchMap`, потому что старые запросы больше не нужны;
- save с очередью: `concatMap`, если каждое сохранение должно дойти до backend по порядку;
- save без дублей: `exhaustMap`, если повторный click во время in-flight запроса нужно игнорировать;
- `mergeMap` подходит для независимых операций, но может создать гонки и дубли при save.

Сильный ответ называет failure mode: stale search result, отмененный write, двойная отправка или перегрузка backend.
...
Компонент не должен сам искать произвольную форму в DOM и знать backend DTO. Лучше передать ему готовую view model
ошибок или typed control, если это локальный form-field component.

Минимальные требования:

- показывать ошибки только после `touched`, submit attempt или явного внешнего флага;
- иметь стабильные `id` для связи input и error через `aria-describedby`;
- поддерживать server errors отдельно от sync validators;
- не дублировать business validation, если она уже выражена в form schema;
- оставаться presentational, а orchestration submit/loading/error держать выше.

Такой вопрос хорошо показывает, умеет ли кандидат проектировать маленький компонент как часть большого Angular flow.
...
пользователь редактирует локальный draft.

Подход:

- selected id хранить как signal;
- details грузить через `resource()`/`httpResource()` или RxJS, если нужны операторы;
- draft сделать writable state, связанный с выбранным item;
- при смене selected id сбрасывать draft на данные нового item;
- save явно отправляет draft, а не мутирует server data при каждом keypress.

`computed()` не подходит для draft, если пользователь должен менять значение вручную: computed readonly и должен
оставаться чистой производной.
...
Условие: parent мутирует объект пользователя in place, child с OnPush не обновляет имя.
...
Лучше создать новую ссылку:

```ts
this.user = {...this.user, name: nextName};
```

Если состояние хранится в signal:

```ts
this.user.update((user) => ({...user, name: nextName}));
```

`ChangeDetectorRef.detectChanges()` не должен быть первым решением. Он может замаскировать нарушение ownership и
...
subscriptions в обработчиках. Через 20 минут вкладка начинает потреблять слишком много памяти.

Хорошее решение:

- найти все manual `subscribe()`, `setInterval`, DOM listeners и сторонние callbacks;
- перевести template-facing streams на `async` pipe или signal boundary;
- для ручных subscriptions использовать `takeUntilDestroyed()` рядом с местом подписки;
- закрывать WebSocket/service resources при logout, а не только при destroy компонента;
- проверить heap snapshots и Angular DevTools Profiler после фикса.

Отдельно стоит обработать ошибки каждого stream локально. Один упавший источник не должен превращать весь dashboard в
пустой экран.
...
- `/admin` вынести в lazy route, потому что это отдельная feature boundary;
- график внутри checkout отложить через `@defer`, например по viewport или interaction;
- заранее продумать placeholder/loading state с устойчивой высотой, чтобы не получить layout shift;
- измерить, не создает ли дробление waterfall на слабой сети;
- для SSR/hydration проверить, что deferred block не ломает first paint и interactivity.

Lazy loading не равно performance автоматически. Оно помогает, когда отложенный код не нужен на критическом пути.
...
1. Сначала измерить: Angular DevTools Profiler, Chrome Performance, production build.
2. Проверить стабильные keys: `@for (row of rows(); track row.id)`.
3. Убрать тяжелые template methods и impure pipes.
4. Вынести derived data в `computed()` или подготовленную view model.
5. Для больших однотипных списков использовать CDK virtual scroll.
6. Проверить, не пересоздаются ли массивы/handlers на каждый render без необходимости.

Важно не начинать с микротюнинга. Если bottleneck в API, сортировке на клиенте или layout thrashing, один `track` не
спасет интерфейс.
...
- найти места, где UI обновляется через обычные поля из сторонних callbacks;
- заменить локальное состояние на signals или явно вызвать `markForCheck()` на интеграционной границе;
- пересмотреть `NgZone.runOutsideAngular()` и ручные `detectChanges()`;
- проверить third-party libraries, которые рассчитывают на Zone.js;
- покрыть сценарии async UI тестами, где ожидание идет через stable state, а не через случайные sleeps.

В zoneless режиме обновления должны приходить из понятных источников: signals, template events, `AsyncPipe`, explicit
...
Условие: приложение выросло, две команды постоянно конфликтуют в одних service и shared folders.

Хороший план:

- выделить domain/feature routes и lazy-loaded boundaries;
- определить owners для feature folders;
- оставить shared layer маленьким и dependency-light;
- запретить feature-to-feature imports без публичного contract;
- вынести reusable UI в library/design system, а product-specific logic оставить в feature;
- договориться о правилах state ownership, DTO mapping и public API.

Цель не в количестве папок, а в том, чтобы команды могли менять свою область без случайной связности с соседней.
